...
2. **조회**: 소스별로 아래 "소스 상세"의 방법으로 받아온다.
   - 검색광고·GFA(RSS/XML)·카카오(JSON)는 `curl -s`로 받아 파싱.
   - HTML 페이지(Google·Meta)는 WebFetch로 본문 추출.
...
- **카카오 데브톡** (Discourse JSON 엔드포인트)
  `curl -sL https://devtalk.kakao.com/c/notice.json`
  → `topic_list.topics[]`: `title` / `slug` / `id` / `created_at` / `excerpt`. 원문 URL = `https://devtalk.kakao.com/t/{slug}/{id}`.
  ⚠️ `/c/notice.json`은 `/c/notice/108.json`으로 **301 리다이렉트**하므로 반드시 `-L`(리다이렉트 추적)을 붙인다. `-L` 없으면 빈 응답(`size=0`)이 온다.
  ⚠️ 목록의 `excerpt`는 **대부분 비어 있다.** 요약 대상 항목의 본문이 비었으면 개별 토픽 JSON
  `curl -sL "https://devtalk.kakao.com/t/{slug}/{id}.json"`을 추가로 받아 `post_stream.posts[0].cooked`(본문 HTML, 태그 제거 후 사용)에서 가져온다. 목록 전체가 아니라 **실제 요약할 항목만** 추가 조회한다(불필요한 호출 방지).

- **네이버 GFA** (Docusaurus 블로그 RSS, XML)
  `curl -s "https://naver-ad-api.github.io/openapi-guide/blog/rss.xml"`
  → `<item>`별 `title` / `link`(글별 고유 URL, 예 `/blog/2026-05-27-update`) / `pubDate` / `description`(HTML 본문).
  ⚠️ `/blog` 목록 페이지를 WebFetch하면 **글별 링크를 못 얻어 원문 링크가 전부 `/blog`로 동일**해진다 → 반드시 RSS(`/blog/rss.xml`)를 쓴다.
